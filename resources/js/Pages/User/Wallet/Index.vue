<template>
    <div class="container mx-auto">
        ...
        <div class="flex gap-x-1 justify-end mt-2">
            <Button @click="open = true" color="primary">Create new transaction</Button>
            <Link :href="route('user.category.index')">
            <Button color="primary">My categories</Button>
            </Link>
        </div>
        ...
        <div class="mt-5 grid grid-cols-2 gap-2">
            ...
            <div class="col-span-2">
                ...
                <h1 class="text-2xl text-center uppercase font-bold my-2">My transaction</h1>
                ...
                <!-- Pagination Section -->
                <div v-if="userTransactions.links && userTransactions.links.length > 1"
    ...
    <Modal @close="(n) => open = n" v-if="open">
        <div ref="modal" class="flex flex-col gap-y-3">
            <form @submit.prevent="createNewTransaction">
                ...
                <p v-if="errors.amount" class="mt-1 text-sm text-red-600 dark:text-red-500">{{ errors.amount }}
                </p>
                <div>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-900">Select a categories</label>
                    <select v-model="formData.category_id" class="w-full rounded-md p-2 bg-white" name="" id="">
                        <option v-for="category in filteredCategories" :value="category.id">{{ category.title }}
                        </option>
                    </select>
...
<script setup>
const props = defineProps({ walletBalance: String, categories: Array, totalIncome: Number, totalExpenses: Number, userTransactions: Object, errors: Object })
